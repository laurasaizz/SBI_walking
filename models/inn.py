...
class CouplingBlock(nn.Module):
    def __init__(
        ...
        self.unchanged_size = (
            input_size // 2 if invert_rounding else (input_size - input_size // 2)
        )  # x1 size   # is half input_size, rounded down
        self.changed_size = (
            input_size - self.unchanged_size
        )  # x2 size  # half, rounded up
        ...
        # self.block_det = 0
        self.register_buffer("orthogonal_rotation_matrix",get_orthonormal_matrix(input_size))
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        # creates views!
        y_drop = y # self.cond_drop(y)  # ENABLED
        ...
        z = z @ self.orthogonal_rotation_matrix
        return z, block_det


    def reverse(self, z, y):
        z_unrotate = z @ self.orthogonal_rotation_matrix.T
        z1, z2 = torch.split(z_unrotate, [self.unchanged_size, self.changed_size], dim=1)
...
class RealNVP(nn.Module):
    ...
    def __init__(
        ...
        for i in range(blocks):  # blocks -1
            self.blocks.append(
                CouplingBlock(
                    input_size, hidden_size, condition_size, invert_rounding=i % 2 == 0
                )
            )
...
class RealNVPSummary(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.summary = RegressionNetwork(
            condition_size,
        ...
        self.realNVP = RealNVP(
            ...
            hidden_size=r_hidden,
            blocks=r_blocks,
            condition_size=reduced_condition_size,
        )
...
class RealNVPsingle(nn.Module):
    ...
    def __init__(
        ...
        for i in range(blocks):
            self.blocks.append(
                CouplingBlockSingle(input_size, hidden_size, condition_size)
            )
