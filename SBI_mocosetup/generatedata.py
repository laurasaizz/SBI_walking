...
def generate_single_sample():
    ...
    solution, model = run_moco_simulation()

    state = model.initSystem()

    # states table from moco solution
    ...
    for j in range(len(labels)):
        ...
        if label.endswith("/value"):
            ...
            if coordSet.contains(coord_name):
                coord_indices.append(j)
                coords.append(coordSet.get(coord_name))
...
def generate_dataset(num_samples, output_file="dataset.npz"):
    ...
    for i in range(num_samples):
        ...
        foot_data, leg = generate_single_sample()
